...
@Component({
  ...
  styles: [`
    ...
    .metric-card.border-pink:hover {
      transform: translateY(-6px) scale(1.025);
      box-shadow: 0 20px 30px -6px rgba(236, 72, 153, 0.28), 0 8px 14px -4px rgba(236, 72, 153, 0.14);
      border-color: #ec4899;
    }
...
export class ComunicacionesComponent implements OnInit {
  ...
  kpiRequierenFirma = computed(() =>
    this.comunicados().filter(c => c.requiereFirma).length
  );

  kpiMensajesSinLeer = computed(() =>
    this.mensajesInbox().filter(m => !m.leido).length
  );
  ...
  comunicadosMovilFiltrados = computed(() => {
    ...
    if (term) {
      list = list.filter(c =>
        (c.titulo && c.titulo.toLowerCase().includes(term)) ||
        (c.imagenes && c.imagenes.some(img => img.texto && img.texto.toLowerCase().includes(term)))
      );
    }
  ...
  mensajesFiltrados = computed(() => {
    ...
    return list.filter(m =>
      ...
      (m.remitente && (m.remitente.nombres.toLowerCase().includes(term) || m.remitente.apellidos.toLowerCase().includes(term))) ||
      (m.destinatario && (m.destinatario.nombres.toLowerCase().includes(term) || m.destinatario.apellidos.toLowerCase().includes(term)))
    );
  });
  ...
  confirmarLecturaPrueba(comunicadoId: string) {
    ...
    this.api.post(`comunicaciones/${comunicadoId}/lecturas`, {
      ...
      next: () => {
        ...
        this.api.get<EstadisticasComunicadoResponse>(`comunicaciones/${comunicadoId}/estadisticas`).subscribe({
  ...
  eliminarComunicado(id: string) {
    this.confirmModalConfig = {
      ...
      message: '¿Está seguro de que desea eliminar este comunicado? Esta acción no se puede deshacer y se borrará de la plataforma escolar.',
  ...
  guardar(publicar: boolean) {
    ...
    this.selectedComunicado.contenido =
      this.selectedComunicado.contenidoHtml || this.selectedComunicado.titulo || 'Comunicado institucional';
