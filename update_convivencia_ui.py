...
config_str = """  readonly modalNuevaActa = signal(false);
  readonly modalActaDetalle = signal(false);
...
methods_str = """
  ...
  reenviarNotificacion(casoId: string) {
    if (confirm('¿Deseas reenviar la notificación al acudiente? Se dejará registro de auditoría.')) {
      ...
      this.api.post<any>(`convivencia/faltas/${casoId}/notificar-acudiente`, { ejecutadoPor: this.authService.getCurrentUser()?.id }).subscribe({
        next: (res) => {
