    ...
    <div class="admin-shell">
      ...
      <aside class="sidebar">
        ...
        <div class="tenant-selector-box">
          <div class="flex-between mb-1">
            ...
            @if (authService.user()?.role === 'SUPER_ADMIN') {
              <button
                (click)="modalNuevoColegio.set(true)"
                class="btn-link-crear-colegio"
          ...
          <div class="tenant-card">
            <div
              class="tenant-avatar"
              [style.background-color]="authService.colegio()?.colorPrimario || '#6366f1'"
            >
            ...
            <div class="tenant-info">
              <span class="tenant-name" [title]="authService.colegio()?.nombre">{{
        ...
        <nav class="nav-menu">
          ...
          <a routerLink="/dashboard" routerLinkActive="active" class="nav-link">
            ...
            <span class="nav-text">
              @if (authService.user()?.role === 'DOCENTE') {
                ...
                authService.user()?.role === 'TESORERO' || authService.user()?.role === 'CONTADOR'
              ) {
                Dashboard Financiero
              } @else if (authService.user()?.role === 'COORDINADOR') {
                Dashboard Coordinación
              } @else if (
                authService.user()?.role === 'ESTUDIANTE' ||
                authService.user()?.role === 'ACUDIENTE'
              ) {
          ...
          @if (
            authService.user()?.role !== 'TESORERO' &&
            authService.user()?.role !== 'CONTADOR' &&
            authService.user()?.role !== 'REVISOR_FISCAL'
          ) {
            <div class="nav-section-title">GESTIÓN ACADÉMICA & LMS</div>
            @if (
              authService.isModuloActivo('M01') &&
              authService.user()?.role !== 'ESTUDIANTE' &&
              authService.user()?.role !== 'ACUDIENTE'
            ) {
            ...
            @if (authService.isModuloActivo('M02')) {
              <a routerLink="/lms" routerLinkActive="active" class="nav-link">
                ...
                <span class="nav-text">
                  {{
                    authService.user()?.role === 'DOCENTE' ||
                    authService.user()?.role === 'ESTUDIANTE'
                      ? 'Aula Virtual & Tareas'
                      : 'Aula Virtual & Tareas LMS'
                  }}
                </span>
                <span
                  class="badge-mini"
                  style="background: rgba(79, 70, 229, 0.2); color: #818cf8; border-color: rgba(79, 70, 229, 0.3);"
                  >LMS</span
                >
              </a>
            }
            ...
            @if (
              authService.isModuloActivo('M06') &&
              (authService.user()?.role === 'RECTOR' ||
                authService.user()?.role === 'COORDINADOR' ||
                authService.user()?.role === 'SUPER_ADMIN' ||
                authService.user()?.role === 'SECRETARIA')
            ) {
          ...
          @if (
            ...
            @if (
              authService.isModuloActivo('M15') &&
              (authService.user()?.role === 'RECTOR' ||
                authService.user()?.role === 'SUPER_ADMIN' ||
                authService.user()?.role === 'TESORERO')
            ) {
              <a routerLink="/talento-humano" routerLinkActive="active" class="nav-link">
                ...
                <span
                  class="badge-mini"
                  style="background: rgba(168, 85, 247, 0.2); color: #c084fc; border-color: rgba(168, 85, 247, 0.3);"
                  >RRHH</span
                >
              </a>
            }
          ...
          @if (
            ...
            @if (
              authService.user()?.role !== 'ESTUDIANTE' && authService.user()?.role !== 'ACUDIENTE'
            ) {
            ...
            @if (
              ...
              @if (authService.isModuloActivo('M09')) {
                <a routerLink="/habeas-data" routerLinkActive="active" class="nav-link">
                  ...
                  <span
                    class="badge-mini"
                    style="background: rgba(2, 132, 199, 0.2); color: #38bdf8; border-color: rgba(2, 132, 199, 0.3);"
                    >1581</span
                  >
                </a>
              }
          ...
          @if (authService.isModuloActivo('M11')) {
            ...
            <a routerLink="/comunicaciones" routerLinkActive="active" class="nav-link">
              ...
              <span
                class="badge-mini"
                style="background: rgba(239, 68, 68, 0.2); color: #f87171; border-color: rgba(239, 68, 68, 0.3);"
                >CIRCULAR</span
              >
            </a>
          }
          ...
          @if (
            ...
            @if (authService.isModuloActivo('M14')) {
              <a routerLink="/porteria" routerLinkActive="active" class="nav-link">
                ...
                <span
                  class="badge-mini"
                  style="background: rgba(52, 211, 153, 0.2); color: #34d399; border-color: rgba(52, 211, 153, 0.3);"
                  >QR</span
                >
              </a>
            }
            @if (authService.isModuloActivo('M16')) {
              <a routerLink="/transporte-restaurante" routerLinkActive="active" class="nav-link">
                ...
                <span
                  class="badge-mini"
                  style="background: rgba(56, 189, 248, 0.2); color: #38bdf8; border-color: rgba(56, 189, 248, 0.3);"
                  >GPS</span
                >
              </a>
            }
          ...
          @if (authService.user()?.role === 'SUPER_ADMIN') {
            ...
            <a
              routerLink="/super-admin/modulos-colegios"
            ...
            >
              ...
              <span
                class="badge-mini"
                style="background: rgba(245, 158, 11, 0.2); color: #fbbf24; border-color: rgba(245, 158, 11, 0.4);"
                >GLOBAL</span
              >
            </a>
            <a routerLink="/permisos" routerLinkActive="active" class="nav-link">
              ...
              <span
                class="badge-mini"
                style="background: rgba(99, 102, 241, 0.2); color: #a5b4fc; border-color: rgba(99, 102, 241, 0.3);"
                >RBAC</span
              >
            </a>
          }
        ...
        <div class="sidebar-footer">
          <div class="footer-top-row">
            ...
            <a href="/docs" target="_blank" class="swagger-link">
              <span>📚 Swagger</span>
            </a>
          </div>
          <button
            ...
            title="Cerrar sesión en EduCoreOS"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
        ...
        <header class="topbar">
          <div class="topbar-left">
            ...
            <span class="system-status"
              >Modo Multitenant: <strong>{{ authService.colegio()?.slug }}</strong></span
            >
          </div>
            ...
            <div class="user-profile-widget">
              <div class="user-details">
                <span class="user-name"
                  >{{ authService.user()?.primerNombre }}
                  {{ authService.user()?.primerApellido }}</span
                >
...
export class AdminLayoutComponent {
  ...
  onColegioChange(event: Event) {
    ...
    const selected = this.authService
      .colegiosDisponibles()
      .find((c) => c.id === selectElement.value);
