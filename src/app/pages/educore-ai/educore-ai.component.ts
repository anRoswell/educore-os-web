    ...
    <div class="ai-page-container animate-fade-in">
      ...
      <div class="page-header">
        <div>
          ...
          <p class="subtitle">
            Modelos generativos anclados en el PEI institucional, redactor automático de boletines
            (Dec. 1290) y analítica predictiva de deserción escolar.
          </p>
        </div>

        <div class="header-actions">
          <button
            (click)="abrirModalConfigPrompts()"
            class="btn btn-secondary"
            title="Configurar directivas del modelo y SIEE"
          >
            <span>⚙️ Configurar Prompts & SIEE</span>
          </button>
          <button
            (click)="abrirModalIndexarPei()"
            class="btn btn-secondary"
            title="Indexar documentos curriculares en la base RAG"
          >
          ...
          <button
            ...
            title="Nueva sesión de conversación"
          >
          ...
          >
                ...
                @for (msg of messages(); track msg.timestamp) {
                  ...
                  >
                    <div class="bubble-header">
                      <strong>{{
                        msg.sender === 'user' ? 'Tú (Docente/Rector)' : 'EduCore AI (RAG)'
                      }}</strong>
            ...
            <div class="card p-4">
              ...
              <div class="sugerencias-list mt-3">
                <button
                  (click)="
                    ejecutarPromptSugerido(
                      '¿Cuáles son las causales de pérdida de año según el SIEE y el manual?'
                    )
                  "
                ...
                <button
                  (click)="
                    ejecutarPromptSugerido(
                      '¿Cuál es el debido proceso para faltas Tipo II según la Ley 1620?'
                    )
                  "
                  class="sugerencia-btn"
                >
                ...
                <button
                  (click)="
                    ejecutarPromptSugerido(
                      '¿Qué adaptaciones curriculares exige el Decreto 1421 para estudiantes PIAR?'
                    )
                  "
                  class="sugerencia-btn"
                >
                ...
                <button
                  (click)="
                    ejecutarPromptSugerido(
                      '¿Cómo se calcula el porcentaje de inasistencia para reprobación?'
                    )
                  "
                  class="sugerencia-btn"
                >
              ...
              <div class="ai-box mt-4">
                ...
                <p class="ai-text">
                  Todas las respuestas son contrastadas con los documentos institucionales oficiales
                  registrados en la base vectorial del colegio.
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="card p-4" style="max-width: 800px; margin: 0 auto;">
            <div class="card-title-bar">
              <div>
                ...
                <p>
                  Genera descripciones cualitativas personalizadas ancladas en la escala nacional
                  (Superior, Alto, Básico, Bajo - Decreto 1290).
                </p>
              </div>
            </div>
                ...
                <span
                  ...
                  {{
                    isGeneratingNarrative()
                      ? 'Redactando con IA...'
                      : 'Generar Observación Pedagógica'
                  }}</span
                >
              </button>
              ...
              @if (narrativaGenerada()) {
                <div class="narrativa-output mt-4 animate-fade-in">
                  <div class="flex-between mb-2">
                    ...
                    <button (click)="copiarNarrativa()" class="btn btn-secondary btn-sm">
                      📋 Copiar Texto
                    </button>
                  </div>
          ...
          <div class="section-intro card">
            <div class="flex-between">
              <div>
                ...
                <p class="text-sm">
                  Algoritmo predictivo que cruza calificaciones reprobadas (< 3.0), inasistencias
                  injustificadas y reportes de convivencia.
                </p>
              </div>
                  ...
                  <tr>
                    <td>
                      ...
                      <div class="text-xs text-slate-500">
                        Doc. {{ est.documento }} • Grado {{ est.grado }}
                      </div>
                    </td>
                    <td>
                      <span
                        class="badge"
                        [class.badge-danger]="est.nivelRiesgo === 'CRITICO'"
                        [class.badge-warning]="est.nivelRiesgo === 'MEDIO'"
                        [class.badge-success]="est.nivelRiesgo === 'BAJO'"
                      >
                        {{ est.nivelRiesgo }}
                      </span>
                    </td>
                    <td>
                      <strong [style.color]="est.scoreRiesgo >= 70 ? '#ef4444' : '#f59e0b'"
                        >{{ est.scoreRiesgo }}/100</strong
                      >
                    </td>
                    <td>
                    ...
                    <td style="text-align: right;">
                      <div class="actions-group">
                        <button
                          (click)="verDetalleDiagnostico(est)"
                          class="btn btn-secondary btn-sm"
                          title="Ver Diagnóstico Integral"
                        >
                          🔍 Diagnóstico
                        </button>
                        <button
                          (click)="redactarParaEstudiante(est.matriculaId)"
          ...
          <div class="section-intro card">
            <div class="flex-between">
              <div>
                ...
                <p class="text-sm">
                  Personalización de plantillas de sistema (System Prompts) para adaptaciones
                  curriculares, boletines y planeación docente.
                </p>
              </div>
              ...
              <tbody>
                @for (p of promptTemplates(); track p.id) {
                  <tr>
                    ...
                    <td>
                      <span class="text-sm">{{ p.descripcion }}</span>
                    </td>
                    <td>
                      <code>{{ p.temperatura }}</code>
                    </td>
                    <td>
        ...
        <div class="modal-backdrop animate-fade-in">
          <div class="modal-card card card-glass" style="max-width: 540px;">
            ...
            <div class="modal-body">
              <div class="form-group">
                ...
                <input
                  type="text"
                  class="form-control"
                  [(ngModel)]="configPromptForm.nombre"
                  placeholder="Ej: Redactor Estándar SIEE 2026"
                />
              </div>
              ...
              <div class="form-group mt-3">
                <label class="form-label"
                  >Temperatura del Modelo: {{ configPromptForm.temperatura }}</label
                >
                <input
                  type="range"
                  min="0.1"
                  ...
                  step="0.1"
                  class="w-full"
                  [(ngModel)]="configPromptForm.temperatura"
                />
                <span class="text-xs text-slate-500"
                  >Valores bajos (0.2) para rigor normativo; valores altos (0.7) para creatividad
                  pedagógica.</span
                >
              </div>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button (click)="modalConfigPrompts.set(false)" class="btn btn-secondary">
                Cancelar
              </button>
            </div>
          </div>
        ...
        <div class="modal-backdrop animate-fade-in">
          <div class="modal-card card card-glass" style="max-width: 520px;">
            ...
            <div class="modal-body">
              <div class="form-group">
                ...
                <input
                  type="text"
                  class="form-control"
                  [(ngModel)]="indexarPeiForm.titulo"
                  placeholder="Ej: Manual de Convivencia — Capítulo 8: Debido Proceso"
                />
              </div>
            ...
            <div class="modal-footer">
              ...
              <button (click)="modalIndexarPei.set(false)" class="btn btn-secondary">
                Cancelar
              </button>
            </div>
          </div>
      ...
      @if (diagnosticoSeleccionado()) {
        <div class="modal-backdrop animate-fade-in">
          <div class="modal-card card card-glass" style="max-width: 540px;">
            ...
            <div class="modal-body">
              <div
                class="student-risk-summary p-3"
                style="background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;"
              >
                <h4 style="color: #0f172a; margin-bottom: 0.25rem;">
                  {{ diagnosticoSeleccionado()?.nombreEstudiante }}
                </h4>
                <p class="text-sm text-slate-500">
                  Documento: {{ diagnosticoSeleccionado()?.documento }} • Grado:
                  {{ diagnosticoSeleccionado()?.grado }}
                </p>
                <div class="badge badge-danger mt-2" style="font-size: 0.85rem;">
                  Nivel de Riesgo: {{ diagnosticoSeleccionado()?.nivelRiesgo }} (Score:
                  {{ diagnosticoSeleccionado()?.scoreRiesgo }}/100)
                </div>
              </div>

              <div class="risk-factors-detail mt-3">
                <h5 style="font-size: 0.9rem; margin-bottom: 0.5rem;">
                  Factores Identificados por el Modelo:
                </h5>
                <ul style="padding-left: 1.25rem; font-size: 0.85rem; color: #475569;">
                  <li>
                    Materias con Desempeño Bajo:
                    <strong>{{ diagnosticoSeleccionado()?.materiasBajo }}</strong>
                  </li>
                  <li>
                    Inasistencias Injustificadas:
                    <strong>{{ diagnosticoSeleccionado()?.inasistencias }}</strong>
                  </li>
              ...
              <div class="ai-box mt-3">
                ...
                <p class="ai-text">
                  Concertar reunión urgente con acudientes, remitir a orientación escolar y
                  programar talleres de nivelación según el Artículo 15 del SIEE.
                </p>
              </div>
            </div>

            <div class="modal-footer">
              <button (click)="diagnosticoSeleccionado.set(null)" class="btn btn-secondary">
                Cerrar
              </button>
            </div>
          </div>
...
export class EducoreAiComponent implements OnInit {
  ...
  // KPIs Computados
  totalConsultasRAG = computed(() => this.messages().filter((m) => m.sender === 'user').length);
  totalNarrativasGeneradas = computed(() => (this.narrativaGenerada() ? 1 : 0));
  totalAlumnosRiesgo = computed(
    () => this.estudiantesRiesgoList().filter((e) => e.scoreRiesgo >= 50).length,
  );
  totalDocumentosIndexados = computed(() => this.promptTemplates().length);
      ...
      next: (res) => {
        if (res && Array.isArray(res) && res.length > 0) {
          const mapped: SearchableOption[] = res.map((e) => ({
            ...
            label:
              `${e.primer_apellido} ${e.segundo_apellido || ''} ${e.primer_nombre} ${e.segundo_nombre || ''}`.trim(),
            sublabel: `Doc. ${e.numero_documento} • Grado: ${e.grado_nombre || 'Sin grado'} (${e.grupo_nombre || 'Sin grupo'})`,
            badge: e.grupo_nombre || 'Sin grupo',
            badgeClass: 'badge-primary',
            avatarText:
              `${e.primer_nombre?.charAt(0) || 'E'}${e.primer_apellido?.charAt(0) || 'S'}`.toUpperCase(),
          }));
    ...
    this.api.post<any>('ai/asistente/consultar-pei', { pregunta: query }).subscribe({
      next: (res) => {
        ...
        this.messages.update((msgs) => [
          ...
          {
            sender: 'ai',
            text: res?.respuesta || 'El servicio no devolvió una respuesta.',
            source: res?.fuenteInstitucional || 'Sin fuente registrada',
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          },
        ]);
      },
    ...
    this.api.post<any>('ai/boletines/redactar-narrativa', dto).subscribe({
      next: (res) => {
        ...
        this.narrativaGenerada.set(
          res?.narrativaGenerada ||
            res?.observacionGenerada ||
            'No se generó una narrativa para este estudiante.',
        );
  ...
  guardarIndexarPei(): void {
    ...
    this.toast.success(
      `Documento "${this.indexarPeiForm.titulo}" vectorizado e indexado en base RAG.`,
    );
  }
}
