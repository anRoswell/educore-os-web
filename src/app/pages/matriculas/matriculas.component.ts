    ...
    <div class="matriculas-container">
      ...
      <div class="page-header">
        <div>
          ...
          <p>
            Directorio escolar 360°, control de expediente documental, formalización y retiro SIMAT
          </p>
        </div>
          ...
          <table class="data-table">
            ...
            <tbody>
              @for (est of filteredEstudiantes(); track est.id) {
                <tr>
                  <td>
                    <strong class="text-indigo-600">{{ est.codigoEstudiante }}</strong>
                  </td>
                  <td>
                    <strong>{{ est.primerApellido }} {{ est.segundoApellido || '' }}</strong
                    >, {{ est.primerNombre }} {{ est.segundoNombre || '' }}
                  </td>
                  <td>
                    <span class="font-mono text-xs"
                      >{{ est.tipoDocumento }} {{ est.numeroDocumento }}</span
                    >
                  </td>
                  <td>
                    <span class="badge badge-info">{{ est.grado }} - {{ est.grupo }}</span>
                  </td>
                  <td>
                    <span class="badge badge-purple"
                      >{{ est.grupoSanguineoRh || 'O+' }} | {{ est.eps || 'N/A' }}</span
                    >
                  </td>
                  <td>
                  ...
                  <td>
                    <div class="actions-group">
                      ...
                      <button
                        (click)="descargarCertificado(est.id)"
                      ...
                      @if (est.estado === 'MATRICULADO' || est.estado === 'ACTIVO') {
                        <button
                          (click)="abrirModalRetiro(est)"
                          class="btn btn-danger btn-sm"
                          title="Retirar / Eliminar"
                        >
                          🗑️ Retiro
                        </button>
                      }
        ...
        <div class="modal-backdrop animate-fade-in">
          <div class="modal-card card card-glass" style="max-width: 680px;">
            ...
            <div class="modal-body">
              <div class="profile-summary mb-4">
                <div class="avatar-large">
                  {{ selectedEstudiante()?.primerNombre?.charAt(0)
                  }}{{ selectedEstudiante()?.primerApellido?.charAt(0) }}
                </div>
                <div>
                  <h4>
                    {{ selectedEstudiante()?.primerNombre }}
                    {{ selectedEstudiante()?.segundoNombre || '' }}
                    {{ selectedEstudiante()?.primerApellido }}
                    {{ selectedEstudiante()?.segundoApellido || '' }}
                  </h4>
                  <p class="text-slate-500 text-sm">
                    Código:
                    <strong class="text-indigo-600">{{
                      selectedEstudiante()?.codigoEstudiante
                    }}</strong>
              ...
              <div class="details-grid">
                <div class="detail-item">
                  ...
                  <strong
                    >{{ selectedEstudiante()?.grado }} - {{ selectedEstudiante()?.grupo }}</strong
                  >
                </div>
                <div class="detail-item">
              ...
              <div class="upload-section mt-4">
                ...
                <p class="text-xs text-slate-500">
                  Cargue copia de documento, registro civil o certificados anteriores.
                </p>
                ...
                @if (uploadSuccess()) {
                  <span class="badge badge-success mt-2"
                    >✓ Soporte documental indexado en el expediente</span
                  >
                }
                <div class="documentos-lista mt-3">
                  <h6 class="text-xs font-bold text-slate-700">
                    📑 Documentos Indexados en el Expediente:
                  </h6>
                  <div class="space-y-1 mt-2">
                    @for (doc of documentosEstudiante(); track doc.id) {
                      <div
                        class="flex items-center justify-between p-2 bg-white rounded border text-xs"
                      >
                        <span>📄 {{ doc.nombreArchivo }} ({{ doc.tipoDocumento }})</span>
                        <div class="flex items-center gap-1">
                          <button
                            (click)="abrirVisorDocumento(doc.urlArchivo, doc.nombreArchivo)"
                            class="btn btn-secondary btn-xs btn-ver-documento"
                          >
                            👁️ Ver
                          </button>
                          <a
                            [href]="resolveUrl(doc.urlArchivo)"
                            target="_blank"
                ...
                <div class="carnet-card-sim mt-2">
                  ...
                  >
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                      <div
                        class="carnet-logo-mini"
                        style="width: 24px; height: 24px; border-radius: 4px; overflow: hidden; background: #ffffff; display: flex; align-items: center; justify-content: center;"
                      >
                        @if (authService.colegio()?.logoUrl) {
                          <img
                            [src]="authService.colegio()?.logoUrl"
                            alt="Logo"
                            style="width: 100%; height: 100%; object-fit: cover;"
                          />
                        } @else {
                          <span style="color: #4f46e5; font-size: 0.6rem; font-weight: bold;"
                            >IE</span
                          >
                        }
                      </div>
                  ...
                  <div class="carnet-body">
                    <div
                      class="carnet-photo-box"
                      style="width: 50px; height: 50px; border-radius: 6px; overflow: hidden; background: #334155; display: flex; align-items: center; justify-content: center; flex-shrink: 0;"
                    >
                      <img
                        [src]="resolveUrl(fotoCarnetUrl())"
                        alt="Foto Carnet"
                        style="width: 100%; height: 100%; object-fit: cover;"
                      />
                    </div>
                    ...
                    <div class="carnet-data">
                      <strong
                        >{{ selectedEstudiante()?.primerNombre }}
                        {{ selectedEstudiante()?.primerApellido }}</strong
                      >
                      <span class="text-xs text-slate-300"
                        >Cód: {{ selectedEstudiante()?.codigoEstudiante }}</span
                      >
                      <span class="text-xs text-slate-300"
                        >Doc: {{ selectedEstudiante()?.numeroDocumento }}</span
                      >
                      <span class="badge badge-info mt-1" style="width: fit-content;"
                        >{{ selectedEstudiante()?.grado }} - {{ selectedEstudiante()?.grupo }}</span
                      >
                    </div>
                  </div>
                  <div
                    class="carnet-footer-contacts"
                    style="background: rgba(0, 0, 0, 0.2); padding: 0.35rem 0.6rem; font-size: 0.65rem; color: #cbd5e1; text-align: center; border-radius: 0 0 8px 8px;"
                  >
                    📍 {{ authService.colegio()?.direccion || 'Sede Principal' }} • 📞
                    {{ authService.colegio()?.telefonoContacto || '(601) 341-2000' }} • ✉️
                    {{ authService.colegio()?.emailContacto || 'info@colegio.edu.co' }}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              <button
                (click)="descargarCertificado(selectedEstudiante()!.id)"
      ...
      @if (modalNuevaMatricula()) {
        <div class="modal-backdrop animate-fade-in">
          <div class="modal-card card card-glass" style="max-width: 620px;">
            <div class="modal-header">
              <div>
                ...
                <span class="modal-subtitle"
                  >Registro en el directorio oficial y asignación de grupo / carnet</span
                >
              </div>
              ...
              <div class="form-grid-2">
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="nuevoEstudiante.primerNombre"
                ...
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="nuevoEstudiante.segundoNombre"
                ...
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="nuevoEstudiante.primerApellido"
                ...
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="nuevoEstudiante.segundoApellido"
                ...
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="nuevoEstudiante.numeroDocumento"
                    placeholder="1025896321"
                  />
                </div>
                  ...
                  <select class="form-select" [(ngModel)]="nuevoEstudiante.grupoId">
                    @for (grupo of nuevoGruposFiltrados(); track grupo.id) {
                      <option [value]="grupo.id">
                        {{ grupo.nombre }} (Salón {{ grupo.salon || 'Principal' }})
                      </option>
                    } @empty {
                ...
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="nuevoEstudiante.grupoSanguineoRh"
                    placeholder="O+"
                  />
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="nuevoEstudiante.nombreAcudiente"
                ...
                <div class="form-group">
                  ...
                  <input
                    type="email"
                    class="form-control"
                    [(ngModel)]="nuevoEstudiante.emailAcudiente"
            ...
            <div class="modal-footer">
              <button
                (click)="guardarNuevaMatricula()"
                class="btn btn-primary"
                [disabled]="isSubmitting()"
              >
                💾 {{ isSubmitting() ? 'Formalizando...' : 'Formalizar Matrícula' }}
              </button>
              <button (click)="modalNuevaMatricula.set(false)" class="btn btn-secondary">
                Cancelar
              </button>
            </div>
          </div>
        ...
        <div class="modal-backdrop animate-fade-in">
          <div class="modal-card card card-glass" style="max-width: 580px;">
              ...
              <div class="form-grid-2">
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="estudianteEnEdicion()!.primerNombre"
                  />
                </div>
                <div class="form-group">
                  <label class="form-label">Segundo Nombre</label>
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="estudianteEnEdicion()!.segundoNombre"
                  />
                </div>
                <div class="form-group">
                  <label class="form-label">Primer Apellido *</label>
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="estudianteEnEdicion()!.primerApellido"
                  />
                </div>
                <div class="form-group">
                  <label class="form-label">Segundo Apellido</label>
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="estudianteEnEdicion()!.segundoApellido"
                  />
                </div>
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="estudianteEnEdicion()!.eps"
                  />
                </div>
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="estudianteEnEdicion()!.grupoSanguineoRh"
                  />
                </div>
                <div class="form-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    [(ngModel)]="estudianteEnEdicion()!.telefonoEmergencia"
                  />
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button (click)="estudianteEnEdicion.set(null)" class="btn btn-secondary">
                Cancelar
              </button>
            </div>
          </div>
      ...
      @if (estudianteParaRetirar()) {
        <div class="modal-backdrop animate-fade-in">
          <div class="modal-card card card-glass" style="max-width: 500px;">
            ...
            <div class="modal-body">
              <p>
                ¿Está seguro de formalizar el retiro de
                <strong
                  >{{ estudianteParaRetirar()?.primerNombre }}
                  {{ estudianteParaRetirar()?.primerApellido }}</strong
                >?
              </p>
              ...
              <div class="form-group mt-3">
                ...
                <select class="form-select" [(ngModel)]="causalRetiro">
                  ...
                  <option value="TRASLADO_INSTITUCION">
            ...
            <div class="modal-footer">
              ...
              <button (click)="estudianteParaRetirar.set(null)" class="btn btn-secondary">
                Cancelar
              </button>
            </div>
          </div>
          ...
          <div class="modal-content modal-lg animate-slide-up">
            ...
            <div class="modal-body">
              <div class="alert alert-info">
                Use variables EJS como <code><%= estudianteNombre %></code>,
                <code><%= acudienteNombre %></code> o <code><%= valorMatricula %></code>.
              </div>
            ...
            <div class="modal-footer">
              ...
              <button class="btn btn-primary" (click)="guardarPlantillas()">
                💾 Guardar Plantillas
              </button>
            </div>
          </div>
          ...
          <div class="modal-content animate-slide-up" style="max-width: 460px;">
            <div class="modal-header" style="border: none; padding-bottom: 0;">
              <div style="display: flex; align-items: center; gap: 14px;">
                <div
                  style="width: 44px; height: 44px; border-radius: 50%; background: #e0e7ff; color: #4f46e5; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-size: 1.25rem;"
                >
                  📩
                </div>
            ...
            >
              <button (click)="showConfirmModal.set(false)" class="btn btn-outline">
                Cancelar
              </button>
              <button
                (click)="confirmModalConfig().onConfirm()"
                class="btn"
                [ngClass]="confirmModalConfig().actionClass || 'btn-primary'"
              >
      ...
      @if (modalVisorDocumento().visible) {
        <div class="modal-backdrop" style="z-index: 10500;">
          <div
            ...
            style="max-width: 850px; width: 95%; height: 80vh; display: flex; flex-direction: column;"
          >
            <div class="modal-header flex justify-between items-center p-4 border-b">
              <h3 class="font-bold text-lg text-slate-800">
                📄 {{ modalVisorDocumento().titulo }}
              </h3>
              <div class="flex items-center gap-2">
                <a
                  [href]="modalVisorDocumento().url"
                  target="_blank"
                  download
            ...
            >
              @if (modalVisorDocumento().esImagen) {
                <img
                  [src]="modalVisorDocumento().url"
                  [alt]="modalVisorDocumento().titulo"
                  class="max-h-full max-w-full object-contain rounded shadow"
                />
              } @else if (modalVisorDocumento().esPdf) {
                <iframe
                  [src]="getSafeViewerUrl(modalVisorDocumento().url)"
            ...
            <div class="modal-footer p-3 border-t flex justify-end">
              <button (click)="cerrarVisorDocumento()" class="btn btn-secondary">
                Cerrar Visor
              </button>
            </div>
          </div>
...
export class MatriculasComponent implements OnInit {
  ...
  readonly modalVisorDocumento = signal<{
    visible: boolean;
    url: string;
    titulo: string;
    esPdf: boolean;
  ...
  guardarNuevaMatricula() {
    if (
      !this.nuevoEstudiante.primerNombre ||
      !this.nuevoEstudiante.primerApellido ||
      !this.nuevoEstudiante.numeroDocumento
    ) {
      this.toast.error(
        'Campos Requeridos',
        'Por favor ingrese nombres, apellidos y número de documento del estudiante.',
      );
      return;
    }
    ...
    if (!this.nuevoEstudiante.gradoId || !this.nuevoEstudiante.grupoId) {
      this.toast.error(
        'Grado y Grupo Requeridos',
        'Por favor seleccione el grado y salón donde se matriculará al estudiante.',
      );
      return;
    }
    ...
    this.api.post<any>('matriculas/formalizar', payload).subscribe({
      ...
      error: (err) => {
        ...
        this.toast.error(
          'Error al matricular',
          err?.error?.message || 'No fue posible formalizar la matrícula.',
        );
      },
    });
  }
  ...
  confirmarRetiro() {
    ...
    this.api
      .post(`matriculas/estudiantes/${est.id}/retiro`, { causal: this.causalRetiro })
      .subscribe({
        next: () => {
          this.estudianteParaRetirar.set(null);
          this.toast.warning(
            'Retiro SIMAT Registrado',
            `El estudiante ${est.primerNombre} ${est.primerApellido} ha sido marcado como RETIRADO.`,
          );
          this.cargarEstudiantes();
        },
        error: (err) => {
          this.toast.error(
            'Error al retirar',
            err?.error?.message || 'No fue posible procesar el retiro.',
          );
        },
      });
  }
        ...
        if (est?.id) {
          this.api.post(`matriculas/estudiantes/${est.id}/documentos`, docDto).subscribe({
            next: () => {
              ...
              this.documentosEstudiante.update((list) => [
                { id: `doc-${Date.now()}`, ...docDto },
                ...list,
              ]);
              this.toast.success(
                'Documento Indexado',
                `Se ha cargado ${file.name} al expediente del estudiante.`,
              );
            },
            error: () => {
              this.uploadSuccess.set(true);
  ...
  descargarCertificado(estudianteId: string) {
    ...
    this.toast.info(
      ...
      'Generando certificado de estudio oficial en formato PDF...',
    );
  }
  ...
  exportarSimat() {
    ...
    this.toast.info(
      'Exportando SIMAT',
      'Generando archivo plano oficial de matrículas para el MEN...',
    );
  }
  ...
  abrirModalPlantillas() {
    this.api.get<any[]>('matriculas/plantillas-legales').subscribe({
      next: (plantillas) => {
        ...
        this.plantillaContrato = contrato
          ? contrato.plantillaEjs
  ...
  guardarPlantillas() {
    this.api
      .put('matriculas/plantillas-legales', {
        tipo: 'CONTRATO_PRESTACION_SERVICIOS',
        plantillaEjs: this.plantillaContrato,
      })
      .subscribe();
    this.api
      .put('matriculas/plantillas-legales', { tipo: 'PAGARE', plantillaEjs: this.plantillaPagare })
      .subscribe({
        next: () => {
  ...
  enviarAFirma(matriculaId: string) {
    this.confirmModalConfig.set({
      ...
      message:
        '¿Desea despachar el código OTP al acudiente para firmar el contrato de matrícula y pagaré?',
      ...
      onConfirm: () => {
        ...
        this.api.post<any>(`matriculas/${matriculaId}/enviar-firma`, {}).subscribe({
          next: (res) => {
            this.toast.success(
              'Enviado',
              res.mensaje || 'Código OTP y documentos despachados exitosamente.',
            );
          },
