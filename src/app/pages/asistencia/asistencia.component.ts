...
@Component({
  ...
  styles: [`
    ...
    .metric-card:hover, .stat-card:hover {
      ...
      box-shadow: 0 20px 30px -6px rgba(99, 102, 241, 0.28), 0 8px 14px -4px rgba(99, 102, 241, 0.14);
      border-color: #6366f1;
    }

    .metric-card:active, .stat-card:active {
    ...
    .metric-card.border-green, .stat-card.success {
      border-left: 4px solid #10b981;
      background: linear-gradient(135deg, #ffffff 50%, rgba(16, 185, 129, 0.08) 100%);
    }
    .metric-card.border-green:hover, .stat-card.success:hover {
      transform: translateY(-6px) scale(1.025);
      box-shadow: 0 20px 30px -6px rgba(16, 185, 129, 0.28), 0 8px 14px -4px rgba(16, 185, 129, 0.14);
      border-color: #10b981;
    }
    .metric-card.border-green .metric-icon, .stat-card.success .stat-icon {
      background: rgba(16, 185, 129, 0.12);
      color: #059669;
    }

    .metric-card.border-amber, .stat-card.warning {
      border-left: 4px solid #f59e0b;
      background: linear-gradient(135deg, #ffffff 50%, rgba(245, 158, 11, 0.08) 100%);
    }
    .metric-card.border-amber:hover, .stat-card.warning:hover {
      transform: translateY(-6px) scale(1.025);
      box-shadow: 0 20px 30px -6px rgba(245, 158, 11, 0.28), 0 8px 14px -4px rgba(245, 158, 11, 0.14);
      border-color: #f59e0b;
    }
    .metric-card.border-amber .metric-icon, .stat-card.warning .stat-icon {
      background: rgba(245, 158, 11, 0.14);
      color: #b45309;
    }

    .metric-card.border-red, .stat-card.danger {
      border-left: 4px solid #ef4444;
      background: linear-gradient(135deg, #ffffff 50%, rgba(239, 68, 68, 0.08) 100%);
    }
    .metric-card.border-red:hover, .stat-card.danger:hover {
      transform: translateY(-6px) scale(1.025);
      box-shadow: 0 20px 30px -6px rgba(239, 68, 68, 0.28), 0 8px 14px -4px rgba(239, 68, 68, 0.14);
      border-color: #ef4444;
    }
    .metric-card.border-red .metric-icon, .stat-card.danger .stat-icon {
    ...
    .metric-card.border-blue:hover {
      transform: translateY(-6px) scale(1.025);
      box-shadow: 0 20px 30px -6px rgba(14, 165, 233, 0.28), 0 8px 14px -4px rgba(14, 165, 233, 0.14);
      border-color: #0ea5e9;
    }
    ...
    .metric-card:hover .metric-icon, .stat-card:hover .stat-icon {
      transform: scale(1.18) rotate(-6deg);
    }

    .metric-info, .stat-info {
      flex: 1;
      min-width: 0;
    }
...
export class AsistenciaComponent implements OnInit {
  ...
  estudiantesSalonExcusa = signal<{ matriculaId: string; nombreCompleto: string; documento?: string }[]>([]);
    ...
    this.api.get<any[]>('academico/periodos').subscribe({
      next: (periodos) => {
        if (periodos && periodos.length > 0) {
          const activo = periodos.find((p: any) => p.estado === 'ACTIVO' || p.estado === 'ABIERTO') || periodos[0];
  ...
  cargarCargasDocentes() {
    this.api.get<any[]>('academico/cargas-docentes').subscribe({
      ...
      error: (err) => {
        ...
        this.cargaDocenteSeleccionada.set('');
        this.alumnosLista.set([]);
        const msg = err?.error?.message || 'No se pudieron consultar las cargas docentes asignadas.';
      ...
      next: (items) => {
        ...
        if (items && items.length > 0) {
          const arr: AlumnoAsistencia[] = items.map((i: any) => ({
            ...
            estudianteNombre: i.estudianteNombre || (i.estudiante ? `${i.estudiante.primer_nombre} ${i.estudiante.primer_apellido}` : 'Estudiante'),
  ...
  guardarPlanilla() {
    ...
    this.api.post('asistencia/sesiones/guardar-planilla', dto).subscribe({
      ...
      error: (err) => {
        this.isSaving.set(false);
        const serverMsg = err?.error?.message || err?.message || 'Error al guardar la planilla de asistencia.';
  ...
  radicarExcusa() {
    this.isSaving.set(true);
    const matriculaId = this.nuevaExcusa.matriculaId || (this.estudiantesSalonExcusa().length > 0 ? this.estudiantesSalonExcusa()[0].matriculaId : '11111111-1111-4111-8111-000000000001');
    const payload = {
      matriculaId,
  ...
  private resetFormExcusa() {
    this.nuevaExcusa = {
      matriculaId: this.estudiantesSalonExcusa().length > 0 ? this.estudiantesSalonExcusa()[0].matriculaId : '',
      motivo: 'MEDICA',
      fechaInicio: this.getHoy(),
      fechaFin: this.getHoy(),
      descripcion: '',
      estudianteNombre: this.estudiantesSalonExcusa().length > 0 ? this.estudiantesSalonExcusa()[0].nombreCompleto : ''
    };
  ...
  rechazarExcusa(excusa: ExcusaMedicaItem) {
    if (excusa.id && !excusa.id.startsWith('exc-demo')) {
      this.api.put(`asistencia/excusas/${excusa.id}/rechazar`, { motivoRechazo: 'Soporte ilegible o extemporáneo' }).subscribe({
        next: () => {
