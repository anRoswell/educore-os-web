...
# Add a Tab selector at the top
tabs_html = """
      <!-- Header -->
      <div class="page-header mb-3">
        <div>
          <h1>LMS & Aulas Virtuales</h1>
          <p>Muros interactivos, gestión de tareas, foros y calificaciones formativas.</p>
        </div>
      ...
      @if (currentTab === 'AULAS') {
        <div class="aulas-grid">
          @if(aulas().length === 0) {
            ...
            <div class="grid-cols-1 md:grid-cols-3 gap-4" style="display: grid; grid-template-columns: 1fr 2fr; gap: 2rem;">
              ...
              <!-- Main Content: Muro de Publicaciones -->
              <div class="muro-feed">
                @if (aulaSeleccionada()) {
                  <div class="flex-between mb-3">
            ...
            <div class="modal-content animate-slide-up">
              ...
              <div class="modal-body">
                <div class="form-group mb-3">
                  ...
                  <input type="text" class="form-control" [(ngModel)]="nuevaAula.nombre" placeholder="Ej: Matemáticas 10-A">
                </div>
                <div class="form-group mb-3">
                  <label>Descripción</label>
                  <textarea class="form-control" [(ngModel)]="nuevaAula.descripcion"></textarea>
                </div>
              </div>
              <div class="modal-footer">
                <button class="btn btn-primary w-100" (click)="guardarAula()">Guardar Aula</button>
              </div>
            </div>
          </div>
        }
        ...
        @if(modalPublicacion()) {
          <div class="modal-backdrop">
            <div class="modal-content animate-slide-up">
              <div class="modal-header">
                <h2>Publicar en el Muro</h2>
                <button class="close-btn" (click)="modalPublicacion.set(false)">X</button>
              </div>
              <div class="modal-body">
                <div class="form-group mb-3">
                  <label>Título</label>
                  <input type="text" class="form-control" [(ngModel)]="nuevaPublicacion.titulo">
                </div>
                <div class="form-group mb-3">
                  <label>Contenido</label>
                  <textarea class="form-control" rows="4" [(ngModel)]="nuevaPublicacion.contenido"></textarea>
                </div>
                <div class="form-group mb-3">
                  <label>Enlace de Video (YouTube/Vimeo)</label>
                  <input type="text" class="form-control" [(ngModel)]="nuevaPublicacion.url_adjunta" placeholder="https://youtube.com/watch?v=...">
                </div>
                <div class="form-group mb-3">
                  <label>Tipo</label>
              ...
              <div class="modal-footer">
                <button class="btn btn-primary w-100" (click)="guardarPublicacion()">Publicar</button>
              </div>
            </div>
          </div>
        }
...
"""
content = content.replace("      <!-- Header -->\n      <div class=\"page-header mb-3\">\n        <div>\n          <h1>LMS & Aulas Virtuales</h1>", tabs_html + "      <div style=\"display:none;\">")
